...
module.exports = (input, options, specialoption) => {
  ...
  // file type that have special options:
  if (frontMatterResult.attributes && frontMatterResult.attributes.type) {
    const type = frontMatterResult.attributes.type;
    // the file have a type AND the specialoptions have defined that type:
    if (specialoption[type]) {
      // then use special options:
      options = Object.assign(
        {
          name: "defualtNameSetByFlotFyrToComponentModuleFunc",
          prependJs: [],
